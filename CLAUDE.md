...
| `src/jarvis/text_chat.spec.md` | Typed conversation REPL (`python -m jarvis.text_chat`) for machines without a microphone | Same reply engine as voice; one dialogue memory per session; diary flushed on exit |
...
| `src/jarvis/tools/builtin/coolify.spec.md` | coolify tool: monitor/control the user's self-hosted Coolify server (status, deploy, restart, start, stop) | Self-hosted only; voice-friendly name resolution; raw data out; honest failure |
| `src/jarvis/tools/builtin/teraprint_portal.spec.md` | teraPrintPortal tool: read-only bridge to the user's self-hosted client portal via key-protected `/api/jarvis` endpoint | Self-hosted only; read-only; single bridge endpoint; 6k-char reply cap |
| `src/jarvis/tools/builtin/uptime_kuma.spec.md` | uptimeKuma tool: check/control the user's self-hosted Uptime Kuma (monitor states, uptime, pause/resume) | Self-hosted only; socket.io session per call; voice-friendly name resolution; raw data out; honest failure |
