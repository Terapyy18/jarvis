...
- **Limits**: 20s timeout. **Size guards** (both measured against the lines handed in, existing + new): rewrites with more than `len(existing) + len(new) + 2` lines are rejected as runaway output; rewrites keeping fewer than `ceil((len(existing) + len(new)) * 0.5)` lines are rejected as loss rather than consolidation (a small picker model answering a long input with one summary line would otherwise wipe the node, which compounds on bulk ingestion where a cold branch root is rewritten once per chunk). Fail-open on any error, parse failure, oversized rewrite, collapsed rewrite, or empty rewrite → caller falls back to plain `append_to_node` for each new fact so they still land (a contradiction or an untidy node is recoverable; a silent wipe or hallucinated bloat is not).
